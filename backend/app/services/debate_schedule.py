...
FORMAL_SCHEDULE: list[DebateSegment] = [
    ...
    seg("opening_rag", "opening_prep", "立论前准备 · RAG检索:立论论据库", "检索双方可用的立论事实、案例和通用标准。", 30, "judge", section="pre"),
    seg("aff_opening_discussion", "opening_prep", "立论前准备 · 正方队内讨论(立论)", "正方队内整合论点、分配证据并排除无来源断言。", 45, "affirmative", 1, "pre"),
    seg("neg_opening_discussion", "opening_prep", "立论前准备 · 反方队内讨论(立论)", "反方队内整合论点、分配证据并排除无来源断言。", 45, "negative", 1, "pre"),
    ...
    seg("aff_cross_2", "cross_examination", "攻辩 · 正方二辩提问", "攻方提问30秒：每次只问一个问题，以「请问」开头；不得自答。", 30, "affirmative", 2),
    seg("neg_cross_answer_2", "cross_examination", "攻辩 · 反方二辩回应", "辩方回答60秒：正面回应，不得反问或回避。", 60, "negative", 2),
    seg("neg_cross_2", "cross_examination", "攻辩 · 反方二辩提问", "攻方提问30秒：每次只问一个问题，以「请问」开头；不得自答。", 30, "negative", 2),
    seg("aff_cross_answer_2", "cross_examination", "攻辩 · 正方二辩回应", "辩方回答60秒：正面回应，不得反问或回避。", 60, "affirmative", 2),
    seg("aff_cross_3", "cross_examination", "攻辩 · 正方三辩提问", "攻方提问30秒：每次只问一个问题，以「请问」开头；不得自答。", 30, "affirmative", 3),
    seg("neg_cross_answer_3", "cross_examination", "攻辩 · 反方二辩回应", "辩方回答60秒：正面回应，不得反问或回避。", 60, "negative", 2),
    seg("neg_cross_3", "cross_examination", "攻辩 · 反方三辩提问", "攻方提问30秒：每次只问一个问题，以「请问」开头；不得自答。", 30, "negative", 3),
    seg("aff_cross_answer_3", "cross_examination", "攻辩 · 正方二辩回应", "辩方回答60秒：正面回应，不得反问或回避。", 60, "affirmative", 2),
    seg("aff_summary_1", "segment_summary", "攻辩小结 · 正方一辩", "针对攻辩阶段态势作小结，严禁脱离实际情况背稿。", 120, "affirmative", 1),
    seg("aff_summary_fact", "argument_review", "攻辩环节 · RAG检索:事实核查", "核查正方小结中的事实和引用风险。", 25, "judge"),
    seg("neg_summary_1", "segment_summary", "攻辩小结 · 反方一辩", "针对攻辩阶段态势作小结，严禁脱离实际情况背稿。", 120, "negative", 1),
    seg("neg_summary_fact", "argument_review", "攻辩环节 · RAG检索:事实核查", "核查反方小结中的事实和引用风险。", 25, "judge"),
    ...
    seg("aff_free_team_discussion", "free_prep", "自由辩论前准备 · 正方队内讨论(自由辩)", "正方整理短句攻防、接力顺序和兜底回应。", 45, "affirmative", 3),
    seg("neg_free_team_discussion", "free_prep", "自由辩论前准备 · 反方队内讨论(自由辩)", "反方整理短句攻防、接力顺序和兜底回应。", 45, "negative", 3),
    ...
    seg("aff_closing_discussion", "closing_prep", "总结陈词前准备 · 正方队内讨论(总结)", "正方确认总结陈词中的胜负标准和价值升华。", 40, "affirmative", 4),
    seg("neg_closing_discussion", "closing_prep", "总结陈词前准备 · 反方队内讨论(总结)", "反方确认总结陈词中的胜负标准和价值升华。", 40, "negative", 4),
    seg("closing_frame", "closing_prep", "总结陈词前准备 · 总结框架确认", "锁定四辩总结结构：回应、比较、升华。", 25, "judge"),
    seg("closing_neg4", "closing", "反方四辩总结", "升华主题，梳理全场交锋，重申反方胜负标准。", 240, "negative", 4),
    ...
    seg("closing_aff4", "closing", "正方四辩总结", "升华主题，梳理全场交锋，重申正方胜负标准。", 240, "affirmative", 4),
...
_DEFAULT_HINTS: dict[str, str] = {
        ...
        "opening_statement": (
            ...
            "发言总字数不低于900字（约三分钟朗读时长），结构为：定义→论点一→论点二→论点三→价值升华。"
        ),
        ...
        "cross_examination": "这是盘问/质询：提问方连续抛出「请问」式问题；回应方必须逐条作答，不要跳过。",
        "segment_summary": "这是攻辩小结：梳理本场交锋得失，指出对方2个核心漏洞；控制在4-6句，不要重复已说内容。",
